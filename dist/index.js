...
var NEVER = INVALID;
// src/index.ts
var VulnerabilitySchema = exports_external.object({
  cve: exports_external.string().optional(),
  package: exports_external.string(),
  versionRange: exports_external.string(),
  ...
  categories: exports_external.array(exports_external.enum(["malware", "backdoor", "botnet", "protestware", "adware", "vulnerability"])).min(1),
  cvssScore: exports_external.number().optional(),
  publishedDate: exports_external.string().optional(),
  fixedVersion: exports_external.string().optional(),
  remediation: exports_external.string().optional(),
  references: exports_external.array(exports_external.string()).optional()
});
var ThreatFeedItemSchema = VulnerabilitySchema;
var AnomalyDetectionResultSchema = exports_external.object({
  package: exports_external.string(),
  version: exports_external.string(),
  anomalyScore: exports_external.number(),
  severity: exports_external.enum(["critical", "high", "medium", "low"]),
  description: exports_external.string(),
  indicators: exports_external.array(exports_external.string()).optional(),
...
class SecurityScanner {
  ...
  async fetchThreatIntelligence(packages) {
    if (this.useLocalFeed) {
      return this.getLocalThreatFeed(packages);
    }
    return [];
  }
  getLocalThreatFeed(packages) {
    const localFeed = [
      {
        cve: "CVE-2023-1234",
    ...
    for (const pkg of packages) {
      const vulnerabilities = localFeed.filter((item) => item.package === pkg.name && Bun.semver.satisfies(pkg.version, item.versionRange));
      matchingVulnerabilities.push(...vulnerabilities.map((vuln) => ({
        ...vuln,
        version: pkg.version
      })));
    }
  ...
  async performDependencyAudit(packages) {
    const results = [];
    for (const pkg of packages) {
  ...
  async findVulnerabilities(pkg) {
    ...
    const filtered = vulnerabilities.filter((vuln) => Bun.semver.satisfies(pkg.version, vuln.versionRange));
    this.setCache(cacheKey, filtered);
    return filtered;
  }
  async checkLicenseCompliance(pkg) {
    if (!pkg.license) {
      return [
        {
          package: pkg.name,
  ...
  async performAnomalyDetection(pkg) {
      ...
      return [
        {
          ...
          description: "Package exhibits unusual characteristics that may indicate malicious behavior",
  ...
  prioritizeResults(results) {
    const uniqueResults = Array.from(new Set(results.map((result) => JSON.stringify(result)))).map((str) => JSON.parse(str));
  ...
  determineAdvisoryLevel(severity, categories) {
  ...
  mapResultsToAdvisories(results) {
    return results.map((result) => ({
      level: this.determineAdvisoryLevel(result.severity, result.categories || []),
      package: result.package,
      version: result.version || (result.versionRange ? "Range: " + result.versionRange : undefined),
...
var scannerInstance = new SecurityScanner;
var scanner = {
  version: "1",
  async scan(params) {
